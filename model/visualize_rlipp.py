...
import numpy as np
try:
    from obo_parser import create_term_name_dict, format_term_label
    OBO_SUPPORT = True
except ImportError:
    OBO_SUPPORT = False
    print("Warning: obo_parser not found. Using GO IDs instead of names.")

def plot_rlipp_distribution(df, output_prefix, name_dict=None):
...
def plot_top_terms(df, output_prefix, top_n=20, name_dict=None):
    ...
    if name_dict:
        labels = [format_term_label(term, name_dict, include_id=False, max_length=60)
                  for term in top_positive['term']]
    else:
        labels = top_positive['term']
    ...
    # Use term names if available
    if name_dict:
        labels_neg = [format_term_label(term, name_dict, include_id=False, max_length=60)
                      for term in bottom_negative['term']]
    else:
        labels_neg = bottom_negative['term']
...
def main():
    ...
    if args.obo and OBO_SUPPORT:
        print(f"Loading GO term names from: {args.obo}")
        try:
        ...
        print("Warning: OBO file provided but obo_parser module not available")
        print("Continuing with GO IDs...\n")

    # Create visualizations
    print("Generating visualizations...")
    plot_rlipp_distribution(df, args.output, name_dict)
    plot_top_terms(df, args.output, args.top_n, name_dict)
